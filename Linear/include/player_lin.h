...
class player_lin : public iplayer
{
    private:
    ...
    public:
        player_lin(const std::vector<int>& _gains) : gains(_gains)
        {
    ...
    private:
        ...
        int  make_decision() 
        {
            ...
            for(int piece = 0; piece < 4; piece++)
            {
                ...
                // Where GA algorithm would be
                // Linear controllor instead
                for (size_t i = 0; i < inputs.size(); ++i) {
                    pieceScore[piece] += inputs[i] * gains[i];
                }
